...
if ( ! class_exists('CV_Promo_Box_Line') ) :
 ...
 * @since        Version 1.0
 */
class CV_Promo_Box_Line extends CV_Shortcode {
      ...
      $this->config = array(
         ...
         'attributes' => array(
            ...
            new CV_Shortcode_Select_Control( 'weight', array(
               ...
               'description' => __( 'Specify the weight for this line text.', 'canvys' ),
               'default'     => 'default',
               'options'     => array(
   ...
   public function composer_additional_assets() { ?>
      ...
      <script id="cv-builder-cv_promo_box_line-script">
         (function($){
            "use strict";
            $(document).on( 'cv-composer-load-cv_promo_box_line', function() {
               var $modal = $('#cv-composer-absolute-container').children().last();
               $modal.find('.control-source select').on( 'change', function() {
                  var $this = $(this), val = $this.val(),
                      $textControls = $modal.find('.control-text, .control-weight'),
   ...
   public function callback( $atts, $content = null ) {
      ...
      if ( ! is_array( $cv_promo_box_lines ) ) {
