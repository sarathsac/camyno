;(function($) {
   ...
   var CVPageSlide = function() {
      this.init();
   };

   CVPageSlide.prototype = {

      init: function() {

         // Make sure content sliding is active
         if ( ! $('html').hasClass('full-page-slider-active') ) return;

         // Sanitize the sections
         this.sanitizeSections();

         // Make sure header is stuck
         if ( document.getElementById('header') ) {
            $('#header').addClass('is-stuck');
         }

         // Apply the page slider
         this.applyFullPage();

         // Make sure we start on the first section
         $.fn.fullpage.moveTo(1);

      },

      sanitizeSections: function() {
         $('#body').children().each( function() {
            var $this = $(this);
            if ( $this.hasClass('cv-content-section') ) {
               $this.addClass('section');
            }
            else if ( $this.hasClass('cv-fullwidth-slider') ) {
               $this.addClass('section');
      ...
      getAnchors: function() {
         var counter = 1, anchors = [];
         $('#body').children().each( function() {
            $(this).attr('data-anchor', 'section-'+counter);
            anchors.push('section-'+counter);
            counter++;
         });
         return anchors;
      },
      ...
      getTooltips: function() {
         var toolTips = [];
         if ( cv_page_slide_nav_labels ) {
            _.each( cv_page_slide_nav_labels.split(','), function( element ) {
      ...
      applyFullPage: function() {
         ...
         $('#body').fullpage({
            ...
            navigationTooltips: self.getTooltips(),
            onLeave: $.proxy( self.onLeave, self ),
            onSlideLeave: $.proxy( self.onSlideLeave, self ),
         });
      ...
      onLeave: function( index, nextIndex, direction ) {
         ...
         var $target = $('#body').children().eq(nextIndex-1);

         // Activate animated entrances
         setTimeout( function() {
            $target.find('[data-entrance]').trigger('entrance');
         }, 500 );

         // Show/hide side navigation buttons
         if ( $('body').hasClass('container-layout-free') && $target.hasClass('cv-fullwidth-slider') ) {
            $('#fullPage-nav').css( 'opacity', 0 );
            setTimeout( function() {
               $('#fullPage-nav').addClass('is-disabled');
            }, 260 );
         }
         else {
            $('#fullPage-nav').removeClass('is-disabled');
            setTimeout( function() {
               $('#fullPage-nav').css( 'opacity', 1 );
            }, 10 );
         }
      ...
      onSlideLeave: function( anchorLink, index, slideIndex, direction ) {
         ...
         var $section = $('[data-anchor="section-'+index+'"]'),
             $slide = $section.find('.cv-content-section.slide').eq(slideIndex),
             $otherSlides = $slide.siblings();

         $otherSlides.find('[data-manual-trigger]').trigger('reset-entrance');
         setTimeout( function() {
            $otherSlides.find('[data-manual-trigger]').trigger('entrance');
         }, 700 );
